...
1. **Clone the repository**:
   ```sh
   git clone https://github.com/your-repository/object-detection-yolo-v4.git
   cd object-detection-yolo-v4
   ```
...
```sh
python main_yolo_v4.py --image-path http://webcam.wildwakeski.de/webcam_gross.jpg?ver=1656681652 --include-picture-with-boundingboxes --filter "person,car" --min-confidence 0.1
```
...
The script uses the YOLOv4 model trained on the COCO dataset. If the model is not found in the specified directory, it will automatically download and extract it.
